...
from src.diffusion_model_st import diff_MCDI
import yaml
from src.gnn_embedding import GNNModule,build_graph_from_coordinates
...
class MCDI_base(nn.Module):
    def __init__(self, target_dim, config, device, num_dim):
        ...
        config_gnn = config["GCN"]
        input_dim = 1 if self.is_unconditional == True else 2
        self.diffmodel = diff_MCDI(config_diff, input_dim)


        hidden_dim = config["GCN"]["hidden_dim"]
        self.gnnencoder = GCN(self.num_dim, hidden_dim)
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                if self.is_unconditional == True:
                    diff_input = ( cond_mask * noisy_cond_history[t] +  current_sample)
                    diff_input = diff_input.unsqueeze(1)  # (B,1,K,L)
                else:
                    cond_obs = (cond_mask * observed_data).unsqueeze(1)
                    noisy_target = (current_sample).unsqueeze(1)
    ...
    def forward(self, batch, is_train=1):
        ...
        latent_rep = observed_data.squeeze(1)
        edges = self.build_graph(location)
        edges = edges.to(self.device)
        gnn_output = self.gnnencoder(latent_rep, edges)
        ...
        return loss_func(gnn_output, cond_mask, observed_mask, side_info, is_train)
...
class stMCDI(MCDI_base):
    def __init__(self, config, device, num_dim, target_dim=1):
        super(stMCDI, self).__init__(target_dim, config, device, num_dim)
    ...
    def process_data(self, batch):
        ...
        location = batch["location"].to(self.device).float()
